...
/* custom function */
bool CLyrics::getLRCFromGOMServer(WCHAR *hash, WCHAR *title, WCHAR *artist, WCHAR *album, bool line1, bool line2, bool line3, bool depart)
{
	// 1. 가사 얻기
	WCHAR lowerHash[33];
	for ( int i = 0; i < 32; i++)
	...
	for (int i=0; i<nCnt; i++)
	{
		...
		nowTime = _wtoi(ltBuf);
		if (befTime < nowTime || depart/*multiline*/)
		{
			// 다음 배열로 ㄱㄱ
			befTime = nowTime;
			nlCnt++;
...
bool CLyrics::getLRCFromAlsongServer(WCHAR *hash, bool line1, bool line2, bool line3, bool depart) 
{
	// make Socket
	if (!AfxSocketInit())
    {
...
bool CLyrics::getLRCFromAlsongServerTA(WCHAR *strTitle, WCHAR *strArtist, bool line1, bool line2, bool line3, bool depart)
{
	// make Socket
	if (!AfxSocketInit())
    {
