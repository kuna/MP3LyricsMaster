...
int CUSLTSamsung::getSyncLen(BYTE *usltData)
{
	// SYNC Header : 01 01 D7 7B 61 00 [Cnt - 30 Arr] 00 00 00 40 00 01
	// 1 1 215 123 97 0
	for (int i=0; i<60; i++)
	{
		if (usltData[i] == 1 && usltData[i+1] == 1 && usltData[i+2] == 215 && usltData[i+3] == 123 && usltData[i+4] == 97 && usltData[i+5] == 0)
			return (int)usltData[i+6];
	}

	return -1;
}
bool CUSLTSamsung::isVaildSyncPos(BYTE *usltData, int usltPos)
{
...
bool CUSLTSamsung::getSync(BYTE *usltData, int *timeData, int *usltPos, int usltLen, WCHAR *sync)
{
...
void CUSLTSamsung::SamsungUSLT2Lyrics(BYTE *pUData, int pUDatLen, CString *l_lrc, int *l_time, int *l_cnt)
{
...
void CUSLTSamsung::Lyrics2SamsungUSLT(BYTE *pUData, int *pUDataLen, CString *l_lrc, int *l_time, int l_cnt)	/* some problem occurs - HC Language */
{
	// init Header Bytes
	for (int i=0; i<70; i++)
		pUData[i] = 0x00;	//NULL
...
int CUSLTSamsung::getSSecFromByte(BYTE *pSecData)
{
	int nsec, msec, sec;
	...
	return (msec + 128*sec + nsec*16384 - (119 + 16*128));
}
void CUSLTSamsung::setBYTEFromSsec(BYTE *pSecData, int ssec)
{
	// 역연산을 수행한다
	int _ssec = ssec + (119 + 16*128);
	int msec = _ssec % 128;
